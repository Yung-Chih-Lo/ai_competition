...
# 1. 版本設定與建立輸出目錄
# ------------------------------
version = "5" # 更新版本號，表明使用 XGBoost + CUDA
...
if len(selected_features) < MIN_FEATURES:
    print(f"警告: 基於 95% 閾值選出的特徵少於 {MIN_FEATURES} 個，將使用 Top {MIN_FEATURES}。")
    selected_features = feature_importance_df['feature'].head(MIN_FEATURES).tolist()
elif len(selected_features) > MAX_FEATURES:
    print(f"警告: 基於 95% 閾值選出的特徵多於 {MAX_FEATURES} 個，將使用 Top {MAX_FEATURES}。")
