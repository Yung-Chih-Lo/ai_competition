...
if not test_exists: print(f"警告：測試檔案未找到: {test_file}.")
...
if X_test is not None: X_test = X_test.drop(columns=cols_to_drop_manual, errors='ignore')
...
if X_test is not None: X_test = X_test.drop(columns=cols_to_drop_high_missing_exist, errors='ignore')
gc.collect() # 刪除後清理內存
...
ranked_broker_numeric_cols = [col for col in X_train.columns if ('買超第' in col or '賣超第' in col) and not col.endswith('券商代號') and not col.endswith('均價')]
cols_to_fill_zero.extend(ranked_broker_numeric_cols)
cols_to_fill_zero.extend([ '官股券商_張增減', '官股券商_金額增減(千)','官股券商_買張', '官股券商_賣張', '官股券商_庫存', '官股券商_買金額(千)', '官股券商_賣金額(千)', '官股券商_買筆數', '官股券商_賣筆數'])
...
if cols_to_fill_zero:
    ...
    if X_test is not None: X_test[cols_to_fill_zero] = X_test[cols_to_fill_zero].fillna(0)
...
for col in cols_interest_coverage:
    if col in X_train.columns:
        ...
        if X_test is not None and col in X_test.columns: X_test[col] = X_test[col].fillna(large_value)
...
categorical_features_original_list = ['季IFRS財報_Z等級', '季IFRS財報_DPZ等級'] + [col for col in X_train.columns if col.endswith('券商代號')]
categorical_features_existing = [col for col in categorical_features_original_list if col in X_train.columns] # 獲取實際存在的分類欄位
missing_in_categoricals = {col: X_train[col].isnull().sum() for col in categorical_features_existing if X_train[col].isnull().any()}
...
if missing_in_categoricals:
    ...
    for col in missing_in_categoricals.keys():
        ...
        if X_test is not None and col in X_test.columns: X_test[col] = X_test[col].fillna(fill_value)
else:
    print("\n定義的分類特徵中沒有缺失值。")
...
if X_test is not None: print("Test NaN count:", X_test.isnull().sum().sum())
if X_train.isnull().sum().sum() > 0: print(f"警告：訓練集仍有 {X_train.isnull().sum().sum()} 個缺失值！")
if X_val.isnull().sum().sum() > 0: print(f"警告：驗證集仍有 {X_val.isnull().sum().sum()} 個缺失值！")

# ------------------------------
# 2.3 **重要：** 轉換分類特徵 dtype
# ------------------------------
# 使用 categorical_features_existing
if categorical_features_existing:
    ...
    for col in categorical_features_existing:
        ...
        train_categories[col] = X_train[col].cat.categories # 記錄訓練集的類別
        if col in X_val.columns:
            try:
                # 使用訓練集的 categories 來轉換驗證集
                X_val[col] = pd.Categorical(X_val[col], categories=train_categories[col], ordered=False)
                # 檢查驗證集是否有訓練集中未出現的類別 (轉換後會變 NaN)
                if X_val[col].isnull().any():
                     print(f"警告: 欄位 '{col}' 在驗證集中包含訓練集未出現的類別，已轉換為 NaN。")
                     # 可以選擇填補這些新出現的 NaN (例如用 'MISSING' 類別，如果之前定義過)
                     # if 'MISSING' in train_categories[col]:
                     #     X_val[col] = X_val[col].fillna('MISSING')

            except Exception as e: # 改為更通用的 Exception
...
temp_xgb = xgb.XGBClassifier(
    ...
    random_state=42, n_jobs=-1, tree_method='hist', device='cuda', enable_categorical=True
)
print("實際傳遞給初步訓練的特徵數量:", X_train.shape[1])
# *** 修正：移除 fit 中的 categorical_feature ***
...
MIN_FEATURES = 50
MAX_FEATURES = 2000
if len(selected_features) < MIN_FEATURES: selected_features = feature_importance_df['feature'].head(MIN_FEATURES).tolist()
elif len(selected_features) > MAX_FEATURES: selected_features = feature_importance_df['feature'].head(MAX_FEATURES).tolist()
print(f"\n步驟 4: 已選擇 Top {len(selected_features)} 個特徵。")

X_train_selected = X_train[selected_features].copy() # 使用 .copy() 避免 SettingWithCopyWarning
...
if selected_categorical_features: print(f"選定的特徵中包含 {len(selected_categorical_features)} 個分類特徵。")
del X_train, X_val # 釋放不再需要的完整數據集
gc.collect()
...
single_xgb = xgb.XGBClassifier(
    objective='binary:logistic', eval_metric='aucpr', scale_pos_weight=scale_pos_weight,
    learning_rate=0.05, n_estimators=1000, max_depth=7, subsample=0.8,
...
# *** 修正：直接在 fit 中使用 early_stopping_rounds ***
single_xgb.fit(
    X_train_selected,
    y_train,
    eval_set=eval_set,
    verbose=10                # 每 50 輪打印評估結果
)
...
try: roc_auc_val = roc_auc_score(y_val, y_pred_val_proba)
except ValueError: roc_auc_val = np.nan
...
# 7. 儲存模型和結果
# ------------------------------
# ... (代碼不變, 注意 best_iteration 部分) ...
model_path = os.path.join(output_dir, f"xgb_model_v{version}.json")
single_xgb.save_model(model_path)
print(f"\n模型已儲存至: {model_path}")
results_summary = {
    "model_type": "XGBoost", "device": "cuda", "feature_selection": f"Top {len(selected_features)} features (99% cum importance)",
    "imputation_strategy": "Drop high missing, Zero fill activity/debt/rate NaNs, Median fill others",
    "hyperparameters": single_xgb.get_params(),
    "best_iteration": single_xgb.best_iteration if hasattr(single_xgb, 'best_iteration') else 'N/A (check logs or model dump)',
    "validation_f1": f1_val, "validation_precision": precision_val, "validation_recall": recall_val,
    "validation_roc_auc": roc_auc_val, "validation_auc_pr": auc_pr_val,
    "num_features_used": len(selected_features),
    "categorical_features_used_count": len(selected_categorical_features) # 使用修正後的變數名
}
...
if test_exists:
    ...
    # **重要**: 應用缺失值填補 (使用訓練集的信息)
    if cols_to_fill_zero: X_test[cols_to_fill_zero] = X_test[cols_to_fill_zero].fillna(0)
    for col in cols_interest_coverage:
        if col in X_test.columns: X_test[col] = X_test[col].fillna(large_value)
    if median_imputer and remaining_missing_numeric_cols: # 確保 imputer 被 fit 過
        test_cols_to_impute = [col for col in remaining_missing_numeric_cols if col in X_test.columns]
        if test_cols_to_impute:
             X_test[test_cols_to_impute] = median_imputer.transform(X_test[test_cols_to_impute])
    if missing_in_categoricals: # 填補分類 NaN
         for col in missing_in_categoricals.keys():
             if col in X_test.columns: X_test[col] = X_test[col].fillna('MISSING')

    # 篩選特徵
    missing_cols_test = [col for col in selected_features if col not in X_test.columns]
    if missing_cols_test: raise ValueError(f"測試集缺少以下必要的特徵: {missing_cols_test}")
    X_test_selected = X_test[selected_features].copy() # 使用 .copy()
    ...
    if selected_categorical_features:
        ...
        for col in selected_categorical_features:
             if col in X_test_selected.columns:
                try:
                    # 使用儲存的訓練集類別來轉換和對齊
                    X_test_selected[col] = pd.Categorical(X_test_selected[col], categories=train_categories[col], ordered=False)
                    if X_test_selected[col].isnull().any():
                        print(f"警告: 欄位 '{col}' 在測試集中包含訓練集未出現的類別，已轉換為 NaN。")
                        # if 'MISSING' in train_categories[col]:
                        #     X_test_selected[col] = X_test_selected[col].fillna('MISSING')

                except Exception as e:
                    print(f"處理測試集欄位 '{col}' 時發生錯誤: {e}.")
                    # Fallback 可能會導致不一致，最好能處理錯誤
    ...
    if list(X_train_selected.columns) != list(X_test_selected.columns):
         print("警告：最終用於預測的訓練集和測試集特徵欄位列表不完全匹配。")
         if len(X_train_selected.columns) != len(X_test_selected.columns):
              raise ValueError("最終用於預測的訓練集和測試集特徵數量不一致！")
